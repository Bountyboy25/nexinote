...
export function MiniMap() {
    ...
    useEffect(() => {
        ...
        cards.forEach(card => {
            ...
            ctx.roundRect(card.x * scale + offX, card.y * scale + offY, Math.max(card.width * scale, 4), Math.max(CARD_HEIGHT_ESTIMATE * scale, 3), 2);
            ctx.fill();
        });
    ...
    return (_jsx("div", { className: styles.minimap, children: _jsx("canvas", { ref: canvasRef, className: styles.canvas }) }));
}
